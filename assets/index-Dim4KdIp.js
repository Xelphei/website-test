...
`:""}var Li=ki,Si={dump:Li};function xe(e,n){return function(){throw new Error("Function yaml."+e+" is removed in js-yaml 4. Use yaml."+n+" instead, which is now safe by default.")}}var _i=w,Fi=De,$i=Pe,Ii=Ge,Mi=Ke,Oi=pe,Ni=dn.load,Di=dn.loadAll,Hi=Si.dump,Bi=E,Ri={binary:ze,float:qe,map:Re,null:je,pairs:Je,set:en,timestamp:Qe,bool:Ye,int:Ue,merge:Xe,omap:Ze,seq:Be,str:He},Pi=xe("safeLoad","load"),ji=xe("safeLoadAll","loadAll"),Yi=xe("safeDump","dump"),Ui={Type:_i,Schema:Fi,FAILSAFE_SCHEMA:$i,JSON_SCHEMA:Ii,CORE_SCHEMA:Mi,DEFAULT_SCHEMA:Oi,load:Ni,loadAll:Di,dump:Hi,YAMLException:Bi,types:Ri,safeLoad:Pi,safeLoadAll:ji,safeDump:Yi};async function S(e){const r=await(await fetch(e)).text();return Ui.load(r)}async function qi(e,n,r){const i=(r.heroSelections||[]).map(o=>{const l=o.color||"#18428F";return o.scrollTo?`<a href="#/" data-scroll-to="${o.scrollTo}" class="hero-bar-button gradient-text-hover" style="background-color: ${l}">${o.label}</a>`:o.path?`<a href="#${o.path}" class="hero-bar-button gradient-text-hover" data-circle-expand style="background-color: ${l}">${o.label}</a>`:`<a href="#/" class="hero-bar-button gradient-text-hover" style="background-color: ${l}">${o.label}</a>`}).join("");e.innerHTML=`
  ...
  `;const n=document.getElementById("events-list");try{const r=await fetch(`${Xi}data/events.json`);if(!r.ok){n.innerHTML=`
      ...
      <div class="timeline-container">
        <div class="timeline-track">
          ${i.map(o=>{const l=o.summary||"Untitled Event",{category:a,color:c}=Ji(l),s=et(l),{dateStr:d,timeStr:u,isAllDay:p}=nt(o),h=o.location?`<p class="font-body text-sm text-gray-400 mt-1">${W(o.location)}</p>`:"",m=o.description?`<p class="font-body text-sm text-gray-500 mt-2 line-clamp-2">${W(o.description)}</p>`:"";return`
                <div class="timeline-event" ${o.htmlLink?`onclick="window.open('${it(o.htmlLink)}', '_blank')" style="--event-color: ${c}; cursor: pointer;"`:`style="--event-color: ${c}"`}>
                  <div class="timeline-dot"></div>
                  <div class="timeline-card">
                    <span class="timeline-category">${W(a)}</span>
                    <h3 class="font-heading font-semibold text-primary-dark mt-1">${W(s)}</h3>
    ...
    `}}function W(e){const n=document.createElement("div");return n.textContent=e,n.innerHTML}function it(e){return e.replace(/'/g,"\\'").replace(/"/g,"&quot;")}async function tt(e,n){e.innerHTML=`
    <div class="max-w-6xl mx-auto px-4 py-12" style="margin-top: 60px;">
  ...
  `}function I(e){const n=document.createElement("div");return n.textContent=e,n.innerHTML}async function ot(e,n){e.innerHTML=`
    <div class="max-w-6xl mx-auto px-4 py-12" style="margin-top: 60px;">
  ...
  `;try{const r=await S(`${n}data/gallery.yaml`),t=document.getElementById("gallery-content");if(!r.photos||r.photos.length===0){t.innerHTML='<p class="font-body text-gray-500 text-center py-8">No photos available yet.</p>';return}t.innerHTML=`
    ...
    `,t.querySelectorAll("[data-gallery-index]").forEach(i=>{i.addEventListener("click",()=>{const o=parseInt(i.dataset.galleryIndex);lt(r.photos,o,n)})})}catch{document.getElementById("gallery-content").innerHTML='<p class="text-red-500 font-body">Failed to load gallery.</p>'}}function lt(e,n,r){const t=e[n],i=document.createElement("div");i.className="lightbox-overlay",i.innerHTML=`
    <span class="lightbox-close">&times;</span>
    <img src="${r}images/${t.path}" alt="${J(t.caption)}" />
    <div class="lightbox-caption">${J(t.caption)}</div>
  `,i.addEventListener("click",o=>{(o.target===i||o.target.classList.contains("lightbox-close"))&&i.remove()}),document.addEventListener("keydown",function o(l){l.key==="Escape"&&(i.remove(),document.removeEventListener("keydown",o))}),document.body.appendChild(i)}function J(e){const n=document.createElement("div");return n.textContent=e,n.innerHTML}async function at(e,n,r){e.innerHTML=`
  ...
  `;try{const t=await S(`${n}data/programs.yaml`),i=document.getElementById("program-item-content");let o=null,l=null;for(const a of t.programs){for(const c of a.items)if(c.slug===r){o=c,l=a;break}if(o)break}if(!o){i.innerHTML=`
    ...
    `}catch{const t=document.getElementById("program-item-content");t&&(t.innerHTML='<p class="text-red-500 font-body">Failed to load program details.</p>')}}function P(e){const n=document.createElement("div");return n.textContent=e,n.innerHTML}async function ct(e,n){e.innerHTML=`
  ...
  `;try{const r=await S(`${n}data/programs.yaml`),t=document.getElementById("programs-content");if(!t)return;t.innerHTML=r.programs.map((i,o)=>{const l=o%2===1,a=i.items.map(c=>{let s="",d="";return c.link?s=c.link:c.scrollTo?(s="#/",d=`data-scroll-to="${c.scrollTo}"`):c.slug&&(s=`#/programs/${c.slug}`),`
        ...
        `}).join("")}catch{const r=document.getElementById("programs-content");r&&(r.innerHTML='<p class="text-red-500 font-body">Failed to load programs.</p>')}}function N(e){const n=document.createElement("div");return n.textContent=e,n.innerHTML}async function st(e,n){e.innerHTML=`
  ...
  `;try{const r=await S(`${n}data/contact.yaml`),t=document.getElementById("contact-content");if(!t)return;const i="https://docs.google.com/forms/d/e/1FAIpQLSegzppXQ_TB9Pk50epXR_uFKdXY9guNzy05o45u357XP99b8w/viewform?embedded=true",o=r.description?`<p class="font-body text-gray-600 mb-8 text-center">${ut(r.description)}</p>`:"",l=i&&!i.includes("YOUR_FORM_ID")?`
    ...
    `;t.innerHTML=o+l}catch{const r=document.getElementById("contact-content");r&&(r.innerHTML='<p class="text-red-500 font-body">Failed to load contact information.</p>')}}function ut(e){const n=document.createElement("div");return n.textContent=e,n.innerHTML}const k="/website-test/";let oe=null;async function dt(){const e=await S(`${k}data/site.yaml`);oe=await S(`${k}data/programs.yaml`),document.title=e.name;const n=document.getElementById("app");n.innerHTML=ft(e,oe);const r=document.getElementById("page-content"),t={"/":a=>qi(a,k,e),"/news":a=>Vi(a,k),"/events":a=>rt(a),"/board":a=>tt(a,k),"/gallery":a=>ot(a,k),"/programs":a=>ct(a,k),"/contact":a=>st(a,k),"/404":a=>{a.innerHTML=`
        ...
        </div>`}};for(const a of oe.programs)for(const c of a.items)c.slug&&!c.link&&!c.scrollTo&&(t[`/programs/${c.slug}`]=s=>at(s,k,c.slug));new En(t).init(r);const o=document.getElementById("floating-menu-toggle"),l=document.getElementById("floating-mobile-menu");o&&l&&o.addEventListener("click",()=>{l.classList.toggle("hidden")}),mt(),gt(),xt()}function ft(e,n){var c;const r=e.floatingNav.filter(s=>!s.external),t=e.floatingNav.find(s=>s.external),i=r.map(s=>s.label==="Programs"?pt(s,n):s.scrollTo?`<a href="#/" data-scroll-to="${s.scrollTo}" data-nav-link class="nav-link">${s.label}</a>`:`<a href="#${s.path}" data-nav-link class="nav-link">${s.label}</a>`).join(""),o=t?`<a href="${t.path}" target="_blank" rel="noopener noreferrer" class="nav-link-parent">${t.label}</a>`:"",l=ht(e,n),a=e.socials?e.socials.map(s=>`<a href="${s.url}" target="_blank" rel="noopener noreferrer" class="text-gray-400 hover:text-white transition-colors" aria-label="${s.platform}">${s.platform}</a>`).join(" | "):"";return`
    <!-- Top Navigation -->
  ...
  `}function pt(e,n){const r=n.programs.map(t=>{const i=t.items.map(o=>o.link?`<a href="${o.link}" class="dropdown-sub-item">${o.name}</a>`:o.scrollTo?`<a href="#/" data-scroll-to="${o.scrollTo}" class="dropdown-sub-item">${o.name}</a>`:`<a href="#/programs/${o.slug}" class="dropdown-sub-item">${o.name}</a>`).join("");return`
